...
def euclidean_distance(vehicle, goal_position):
    return sum(((x - goal_position[0]) ** 2 + (y - goal_position[1]) ** 2) ** 0.5 for x, y in vehicle.positions)

def chebyshev_distance(vehicle, goal_position):
    return sum(max(abs(x - goal_position[0]), abs(y - goal_position[1])) for x, y in vehicle.positions)

def blocking_vehicles(state, goal_position):
    vehicle_A_positions = state.vehicles['A'].positions
    blocking_count = 0
    for vehicle in state.vehicles.values():
        if vehicle.id == 'A':
            continue
        for pos in vehicle.positions:
            if any(pos_A[0] < pos[0] < goal_position[0] for pos_A in vehicle_A_positions):
                blocking_count += 1
                break
    return blocking_count

def average_distance_to_vehicles(vehicle, other_vehicles):
    total_distance = 0
    for other_vehicle in other_vehicles:
        total_distance += sum(abs(x - pos[0]) + abs(y - pos[1]) for x, y in vehicle.positions for pos in other_vehicle.positions)
    return total_distance / len(other_vehicles) if other_vehicles else 0

def combined_heuristic(state, goal_position):
    vehicle_A = state.vehicles['A']
    other_vehicles = [v for k, v in state.vehicles.items() if k != 'A']
    h1 = manhattan_distance(vehicle_A, goal_position)
    h2 = euclidean_distance(vehicle_A, goal_position)
    h3 = chebyshev_distance(vehicle_A, goal_position)
    h4 = blocking_vehicles(state, goal_position)
    h5 = average_distance_to_vehicles(vehicle_A, other_vehicles)
    return (h1 + h2 + h3 + h4 + h5) / 5  # Promedio de las cinco heurísticas
    ...
    while heap:
        ...
        for next_state, action in current_state.generar_nuevos_estados():
            if next_state not in visited:
                ...
                heuristic_cost = combined_heuristic(next_state, goal_position)
